class BPFenceKit extends ItemBase
{	
	...
	override void OnPlacementComplete( Man player )
	{
		if ( GetGame().IsServer() )
		{
			...
			BPFence bpfence = BPFence.Cast( GetGame().CreateObject( "BPFence", GetPosition() ) );
			bpfence.SetPosition( position );
			bpfence.SetOrientation( orientation );
			
			//сделать набор невидимым, чтобы его можно было уничтожить при развертывании UA по окончании действия
			HideAllSelections();
